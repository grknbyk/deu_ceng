...
public class HashedDictionary<K, V> implements DictionaryInterface<K, V> {
	...
	private static final int DEFAULT_SIZE = 2447;
	...
	private int PAF(K key) {
		...
		int sum = 0;
		for (int i = 0; i < len; i++)
			sum += str.charAt(i) * Math.pow(7, len - 1 - i);

		return sum;
	}

	// custom hashCode function for String
	...
	// for double hashing h2
	private int secHashFunction(K key) {
		int hashCode = getHashCode(key);
		return 31 - (hashCode % 31);

	}

	private int probeType(K key) {
		// return 1; // linear probing
		return secHashFunction(key); // double hashing
	}

	private int probe(int index, K key) {
		boolean found = false;
		int removedStateIndex = -1;
		int probeType = probeType(key);
		while (!found && (hashTable[index] != null)) {
			if (hashTable[index].isIn()) {
				if (key.equals(hashTable[index].getKey()))
					found = true;
				else
					index = (index + probeType) % hashTable.length;
			} else {
